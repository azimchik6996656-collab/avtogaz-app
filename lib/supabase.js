...
export const storage = {
  async get(key) {
    ...
    const res = await fetch(`/api/data?key=${encodeURIComponent(key)}&branchId=${encodeURIComponent(authClient.branchId)}`, {
      cache: "no-store", // versiya raqami keshlangan (eski) javob bilan hech qachon kelmasin
  ...
  async set(key, valueStr, opts = {}) {
    ...
    const res = await fetch("/api/data", {
      ...
      body: JSON.stringify({ key, valueStr, expectedVersion: opts.expectedVersion, branchId: authClient.branchId }),
    });
    const json = await res.json().catch(() => ({}));
