...
export function issueSession({ role, name, branchId }) {
  if (!SECRET) throw new Error("PIN_SESSION_SECRET sozlanmagan");
  // branchId shu yerda IMZOLANADI — keyinchalik /api/data shu tokendan o'qib,
  // mijoz so'rovga boshqa filial nomini yozib qo'ysa ham e'tiborsiz qoldiradi.
  const payload = { role, name: name || null, branchId: branchId || "main", exp: Date.now() + TTL_MS };
